...
class idAF {
    ...
    fun Load(ent: idEntity, fileName: String): Boolean {
        ...
        while (i < physicsObj.GetNumConstraints()) {
            ...
            while (j < file.constraints.Num()) {
                ...
                if (file.constraints[j].name.Icmp(constraint.GetName()) == 0 && file.constraints[j].type.ordinal == constraint.GetType().ordinal) {
                    break
                }
                j++
            }
    ...
    fun EntitiesTouchingAF(touchList: Array<afTouch_s> /*[ MAX_GENTITIES ]*/): Int {
        ...
        numClipModels = Game_local.gameLocal.clip.ClipModelsTouchingBounds(
            physicsObj.GetAbsBounds(), -1, clipModels, Game_local.MAX_GENTITIES
        )
        i = 0
        while (i < jointMods.Num()) {
            body = physicsObj.GetBody(jointMods[i].bodyId)!!
    ...
    fun UpdateAnimation(): Boolean {
        ...
        animator!!.FinishAFPose(
            modifiedAnim, GetBounds().Expand(POSE_BOUNDS_EXPANSION), Game_local.gameLocal.time
        )
    ...
    fun AddBindConstraints() {
        ...
        while (kv != null) {
            ...
            if (body == null) {
                Game_local.gameLocal.Warning(
                    "idAF::AddBindConstraints: body '%s' not found on entity '%s'", bodyName, self!!.name
                )
                lexer.FreeSource()
            ...
            if (type.Icmp("fixed") == 0) {
                ...
                Game_local.gameLocal.Warning(
                    "idAF::AddBindConstraints: unknown constraint type '%s' on entity '%s'", type, self!!.name
                )
            }
            lexer.FreeSource()
            kv = args.MatchPrefix("bindConstraint ", kv)
    ...
    protected fun SetBase(body: idAFBody, joints: Array<idJointMat>) {
        ...
        AddBody(
            body, joints, animator!!.GetJointName(animator!!.GetFirstChild("origin")), AFJointModType_t.AF_JOINTMOD_AXIS
        )
    }
    ...
    protected fun AddBody(
        body: idAFBody, joints: Array<idJointMat>, jointName: String, mod: AFJointModType_t
    ) {
        val index: Int
    ...
    protected fun LoadBody(fb: idDeclAF_Body, joints: Array<idJointMat>): Boolean {
        ...
        while (i < jointList.Num()) {
            if (jointBody[jointList[i]] != -1) {/*jointHandle_t*/
                Game_local.gameLocal.Warning(
                    "%s: joint '%s' is already contained by body '%s'",
                    name,
                    animator!!.GetJointName(jointList[i]),
            ...
            declAFConstraintType_t.DECLAF_CONSTRAINT_BALLANDSOCKETJOINT -> {
                ...
                when (fc.limit) {
                    ...
                    idDeclAF_Constraint.LIMIT_PYRAMID -> {
                        ...
                        c.SetPyramidLimit(
                            axis[0], axis[1], fc.limitAngles[0], fc.limitAngles[1], fc.shaft[0].ToVec3()
                        )
                    }
    ...
    internal class GetJointTransform private constructor() : getJointTransform_t() {
        override fun run(
            ...
            if ((joint >= 0) && (joint < model.NumJoints())) {
