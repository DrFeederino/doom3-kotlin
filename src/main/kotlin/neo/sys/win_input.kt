...
object win_input {
    ...
    val polled_didod: Array<InputEvent?> = arrayOfNulls<InputEvent?>(DINPUT_BUFFERSIZE) // Receives buffered data
    val s_scantokey: CharArray = charArrayOf(
        0.toChar(),
        27.toChar(),
        '1',
        '2',
    ...
    val s_scantokey_french: CharArray = charArrayOf(
        0.toChar(),
        27.toChar(),
        '1',
        '2',
    ...
    val s_scantokey_german: CharArray = charArrayOf(
        0.toChar(),
        27.toChar(),
        '1',
        '2',
    ...
    val s_scantokey_italian: CharArray = charArrayOf(
        0.toChar(),
        27.toChar(),
        '1',
        '2',
    ...
    val s_scantokey_spanish: CharArray = charArrayOf(
        0.toChar(),
        27.toChar(),
        '1',
        '2',
     ...
     */
    fun IN_DIMapKey(
        key: Int, scancode: Int, mods: Int
    ): Int { // GLFW maps all special/non-printable keys starting from 256
        if (key >= 256) {
            return when (key) {
    ...
    fun Sys_InitInput() {
        ...
        if (Win32Vars_t.in_mouse.GetBool()) {
            IN_InitDIMouse() // don't grab the mouse on initialization
    ...
    fun Sys_InitScanTable() {
        ...
        if (lang.Icmp("english") == 0) {
            keyScanTable = s_scantokey // the only reason that english right alt binds as K_ALT is so that
    ...
    fun Sys_ReturnKeyboardInputEvent(ch: IntArray, action: Int, key: Int, scancode: Int, mods: Int): Int {
        ...
        when (ch[0]) {
            KeyInput.K_BACKSPACE -> { // SE_KEY covers bindings and key-state tracking for all actions.
            ...
            KeyInput.K_PRINT_SCR -> {
                if (action != GLFW.GLFW_RELEASE) { // don't queue printscreen keys.  Since windows doesn't send us key
            ...
            KeyInput.K_CTRL, KeyInput.K_ALT, KeyInput.K_RIGHT_ALT -> win_main.Sys_QueEvent(
            ...
            else -> { // Always queue SE_KEY so binding execution and key-state tracking
                ...
                if (action != GLFW.GLFW_RELEASE && ch[0] in 32..126 && ch[0] != '`'.code && ch[0] != '~'.code) { // Compute the actual typed character including shift/caps state.
                    ...
                    if (typedChar != 0) {
                        win_main.Sys_QueEvent(
                            Instant.now().toEpochMilli(), sysEventType_t.SE_CHAR, typedChar, 0, 0, null
                        )
                    }
                }
            }
