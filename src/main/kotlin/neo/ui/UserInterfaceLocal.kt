...
//	idStr work;
        ...
        override fun WriteToSaveGame(savefile: idFile): Boolean {
            ...
            savefile.WriteInt(len)
            savefile.Write(ByteBuffer.wrap(activateStr.toString().toByteArray()), len)
            len = pendingCmd.Length()
            savefile.WriteInt(len)
            savefile.Write(ByteBuffer.wrap(pendingCmd.toString().toByteArray()), len)
            len = returnCmd.Length()
            savefile.WriteInt(len)
            savefile.Write(ByteBuffer.wrap(returnCmd.toString().toByteArray()), len)
