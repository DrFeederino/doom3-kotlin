...
Original source: neo/renderer/MegaTexture.cpp
...
object MegaTexture {
    ...
    internal class idTextureTile {
        ...
        companion object {
            val SIZE: Int = (Integer.SIZE + Integer.SIZE)
        }
    }
    ...
    internal class idTextureLevel {
        ...
        fun UpdateForCenter(center: FloatArray /*[2]*/) {
            ...
            if (tilesWide <= TILE_PER_LEVEL && tilesHigh <= TILE_PER_LEVEL) {
                ...
                localTileOffset[0] = 0
                localTileOffset[1] = 0 // orient the mask so that it doesn't mask anything at all
                parms.put(0, 0.25f)
                parms.put(1, 0.25f)
                parms.put(3, 0.25f)
            } else {
        ...
        fun UpdateTile(localX: Int, localY: Int, globalX: Int, globalY: Int) {
            ...
            if ((globalX >= tilesWide) || (globalX < 0) || (globalY >= tilesHigh) || (globalY < 0)) { // off the map
            } else { // extract the data from the full image (FIXME: background load from disk)
            ...
            if (idMegaTexture.r_showMegaTextureLabels.GetBool()) { // put a color marker in it
                val color /*[4]*/: ByteArray = byteArrayOf(
                    (255 * localX / TILE_PER_LEVEL).toByte(), (255 * localY / TILE_PER_LEVEL).toByte(), 0, 0
                )
                for (x in 0..7) {
                    for (y in 0..7) {
                        System.arraycopy(
            ...
            while (true) {
                ...
                for (y in 0 until size) {
                    ...
                    for (x in 0 until size) {
                        out[x * 4 + 0] =
                            ((((`in`[x * 8 + 0].toInt() and 0xFF) + (`in`[(x * 8) + 4 + 0].toInt() and 0xFF) + (in2[x * 8 + 0].toInt() and 0xFF) + (in2[(x * 8) + 4 + 0].toInt() and 0xFF)) shr 2)).toByte()
                        out[x * 4 + 1] =
                            ((((`in`[x * 8 + 1].toInt() and 0xFF) + (`in`[(x * 8) + 4 + 1].toInt() and 0xFF) + (in2[x * 8 + 1].toInt() and 0xFF) + (in2[(x * 8) + 4 + 1].toInt() and 0xFF)) shr 2)).toByte()
                        out[x * 4 + 2] =
                            ((((`in`[x * 8 + 2].toInt() and 0xFF) + (`in`[(x * 8) + 4 + 2].toInt() and 0xFF) + (in2[x * 8 + 2].toInt() and 0xFF) + (in2[(x * 8) + 4 + 2].toInt() and 0xFF)) shr 2)).toByte()
                        out[x * 4 + 3] =
                            ((((`in`[x * 8 + 3].toInt() and 0xFF) + (`in`[(x * 8) + 4 + 3].toInt() and 0xFF) + (in2[x * 8 + 3].toInt() and 0xFF) + (in2[(x * 8) + 4 + 3].toInt() and 0xFF)) shr 2)).toByte()
                    }
        ...
        companion object {
            val SIZE: Int = (CPP_class.POINTER_SIZE //idMegaTexture * mega
                    + Integer.SIZE + Integer.SIZE + Integer.SIZE + CPP_class.POINTER_SIZE //idImage * image
    ...
    class idMegaTexture {
        private val levels: Array<idTextureLevel> =
            Array(MAX_LEVELS) { idTextureLevel() } // 0 is the highest resolution
        ...
        fun InitFromMegaFile(fileBase: String): Boolean {
            ...
            // C++: memset( levels, 0, sizeof(levels) ) - objects already default-initialized
            while (true) {
        ...
        fun SetMappingForSurface(tri: srfTriangles_s) {    // analyzes xyz and st to create a mapping
            ...
            val axis: Array<idDrawVert> = Array(2) { idDrawVert() }
            origin.st[0] = 1.0f
            origin.st[1] = 1.0f
            axis[0].st[0] = 0.0f
            axis[0].st[1] = 1.0f
        ...
        class MakeMegaTexture_f private constructor() : cmdFunction_t() {
            override fun run(args: CmdArgs.idCmdArgs?) {
                ...
                // won't fit in memory
                val targa_rgba = ByteArray(TILE_SIZE * targa_header.width * 4)
                ...
                while (blockRowsRemaining-- != 0) {
                    ...
                    if (targa_header.image_type.code == 2 || targa_header.image_type.code == 3) { // Uncompressed RGB or gray scale image
                        row = 0
                        while (row < TILE_SIZE) {
                        ...
                        while (row < TILE_SIZE) {
                            ...
                            breakOut@ while (column < columns) {
                                ...
                                packetSize = 1 + (packetHeader.toInt() and 0x7f)
                    ...
                    for (rowBlock in 0 until mtHeader.tilesWide) {
                        for (y in 0 until TILE_SIZE) {
                            out.Write(
                                ByteBuffer.wrap(
                                    Arrays.copyOfRange(
                                        targa_rgba, (y * targa_header.width + rowBlock * TILE_SIZE) * 4, targa_rgba.size
                                    )
                                ), TILE_SIZE * 4
                            )
                        }
        ...
        companion object {
            val SIZE: Int = (CPP_class.POINTER_SIZE //idFile fileHandle
                    ...
                    + idVec3.SIZE + (java.lang.Float.SIZE * 2 * 4) + Integer.SIZE + (idTextureLevel.SIZE * MAX_LEVELS) + megaTextureHeader_t.BYTES)
            ...
            private fun GenerateMegaMipMaps(header: megaTextureHeader_t, outFile: idFile) {
                ...
                while (width > 1 || height > 1) {
                    ...
                    for (y in 0 until newHeight) {
                        ...
                        Session.session.UpdateScreen()
                        for (x in 0 until newWidth) { // mip map four original blocks down into a single new block
                            for (yy in 0..1) {
                                for (xx in 0..1) {
                                    val tx: Int = x * 2 + xx
                                    val ty: Int = y * 2 + yy
                                    if (tx > width || ty > height) { // off edge, zero fill
                                        newBlock.fill(0)
                                    } else {
                                    ...
                                    for (yyy in 0 until (TILE_SIZE / 2)) {
                                        for (xxx in 0 until (TILE_SIZE / 2)) {
                                            ...
                                            newBlock[out + 0] =
                                                ((((oldBlock[`in` + 0].toInt() and 0xFF) + (oldBlock[`in` + 4].toInt() and 0xFF) + (oldBlock[`in` + 0 + (TILE_SIZE * 4)].toInt() and 0xFF) + (oldBlock[`in` + 4 + (TILE_SIZE * 4)].toInt() and 0xFF)) shr 2)).toByte()
                                            newBlock[out + 1] =
                                                ((((oldBlock[`in` + 1].toInt() and 0xFF) + (oldBlock[`in` + 5].toInt() and 0xFF) + (oldBlock[`in` + 1 + (TILE_SIZE * 4)].toInt() and 0xFF) + (oldBlock[`in` + 5 + (TILE_SIZE * 4)].toInt() and 0xFF)) shr 2)).toByte()
                                            newBlock[out + 2] =
                                                ((((oldBlock[`in` + 2].toInt() and 0xFF) + (oldBlock[`in` + 6].toInt() and 0xFF) + (oldBlock[`in` + 2 + (TILE_SIZE * 4)].toInt() and 0xFF) + (oldBlock[`in` + 6 + (TILE_SIZE * 4)].toInt() and 0xFF)) shr 2)).toByte()
                                            newBlock[out + 3] =
                                                ((((oldBlock[`in` + 3].toInt() and 0xFF) + (oldBlock[`in` + 7].toInt() and 0xFF) + (oldBlock[`in` + 3 + (TILE_SIZE * 4)].toInt() and 0xFF) + (oldBlock[`in` + 7 + (TILE_SIZE * 4)].toInt() and 0xFF)) shr 2)).toByte()
                                        }
                                    }
         ...
         */
            private fun GenerateMegaPreview(fileName: String) {
                ...
                var height: Int = header.tilesHigh
                var tileOffset = 1
                val tileBytes: Int = tileSize * tileSize * 4 // find the level that fits
                ...
                val pic: ByteBuffer = ByteBuffer.allocate(width * height * tileBytes)
    ...
    class fillColors {
        ...
        fun getColor(index: Int): Byte {
            return ((intVal shr (index * 8)) and 0xFF).toByte()
        }

        fun setColor(index: Int, color: Short) {
            val down: Int = 0xFF shl (index * 8)
            intVal = intVal and down.inv()
            intVal = intVal or ((color.toInt() and 0xFF) shl (index * 8))
        }
    }
